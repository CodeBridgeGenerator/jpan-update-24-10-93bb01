...
const Stage2FakerDialogComponent = (props) => {
    ...
    const [stage1Ids, setstage1Ids] = useState();
    ...
    useEffect(() => {
        ...
        client.service("stage1")
                        .find({ query: { $select: ["_id","nomborRujukan"] } })
                        .then((data) => {
                        setstage1Ids(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = stage2FakerFactory(props.user,fakerCount,stage1Ids);
