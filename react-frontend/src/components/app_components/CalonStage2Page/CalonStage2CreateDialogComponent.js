...
const CalonStage2CreateDialogComponent = (props) => {
...
const [tajukLatihan, setTajukLatihan] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [name,tajukLatihan], setError);
        }
        set_entity({...init});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name?._id,tajukLatihan: _entity?.tajukLatihan?._id,
        ...
        const result = await client.service("calonStage2").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info CalonStage2 updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in CalonStage2" });
        }
        setLoading(false);
    };
...
const tajukLatihanOptions = tajukLatihan.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create CalonStage2" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="calonStage2-create-dialog-component">
...
export default connect(mapState, mapDispatch)(CalonStage2CreateDialogComponent);
