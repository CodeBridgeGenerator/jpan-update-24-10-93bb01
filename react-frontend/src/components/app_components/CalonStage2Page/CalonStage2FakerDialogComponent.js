...
import calonStage2FakerFactory from "./calonStage2FakerFactory";
...
const CalonStage2FakerDialogComponent = (props) => {
...
client.service("stage2")
    ...
    const onRun = async () => {
        let fakeData = calonStage2FakerFactory(props.user,fakerCount,nameIds,tajukLatihanIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("calonStage2").create(elem));
...
export default connect(mapState, mapDispatch)(CalonStage2FakerDialogComponent);
