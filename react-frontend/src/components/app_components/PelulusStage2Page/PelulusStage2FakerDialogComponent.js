...
import pelulusStage2FakerFactory from "./pelulusStage2FakerFactory";
...
const PelulusStage2FakerDialogComponent = (props) => {
...
client.service("stage2")
    ...
    const onRun = async () => {
        let fakeData = pelulusStage2FakerFactory(props.user,fakerCount,nameIds,tajukLatihanIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("pelulusStage2").create(elem));
...
export default connect(mapState, mapDispatch)(PelulusStage2FakerDialogComponent);
