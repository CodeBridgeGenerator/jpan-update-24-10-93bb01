...
const [tajukLatihan, setTajukLatihan] = useState([])
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            name: _entity?.name?._id,tajukLatihan: _entity?.tajukLatihan?._id,status: _entity?.status,
        ...
        const result = await client.service("pelulusStage1").create(_data);
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info PelulusStage1 updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in PelulusStage1" });
        }
        setLoading(false);
    };
...
const tajukLatihanOptions = tajukLatihan.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create PelulusStage1" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="pelulusStage1-create-dialog-component">
