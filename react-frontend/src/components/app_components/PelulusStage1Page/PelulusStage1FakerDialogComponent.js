...
import pelulusStage1FakerFactory from "./pelulusStage1FakerFactory";
...
const PelulusStage1FakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [nameIds, setnameIds] = useState();
const [tajukLatihanIds, settajukLatihanIds] = useState();
...
client.service("stage1")
                        .find({ query: { $select: ["_id","tajukLatihan"] } })
                        .then((data) => {
                        settajukLatihanIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = pelulusStage1FakerFactory(props.user,fakerCount,nameIds,tajukLatihanIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("pelulusStage1").create(elem));
...
export default connect(mapState, mapDispatch)(PelulusStage1FakerDialogComponent);
