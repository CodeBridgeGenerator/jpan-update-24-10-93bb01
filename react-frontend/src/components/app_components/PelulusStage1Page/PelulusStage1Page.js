...
import PelulusStage1Datatable from "./PelulusStage1DataTable";
import PelulusStage1EditDialogComponent from "./PelulusStage1EditDialogComponent";
import PelulusStage1CreateDialogComponent from "./PelulusStage1CreateDialogComponent";
import PelulusStage1FakerDialogComponent from "./PelulusStage1FakerDialogComponent";
import PelulusStage1SeederDialogComponent from "./PelulusStage1SeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const PelulusStage1Page = (props) => {
    ...
    const filename = "pelulusStage1.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("pelulusStage1");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , name : urlParams.singleUsersId ,tajukLatihan : urlParams.singleStage1Id  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "PelulusStage1", type: "error", message: error.message || "Failed get PelulusStage1" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("pelulusStage1").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "PelulusStage1", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("pelulusStage1").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/pelulusStage1/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="pelulusStage1-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="pelulusStage1-datatable">
                    <PelulusStage1Datatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <PelulusStage1EditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <PelulusStage1CreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <PelulusStage1FakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <PelulusStage1SeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(PelulusStage1Page);
